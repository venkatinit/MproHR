...
@Component({
  ...
  styleUrls: ['./lwf.component.scss']
})
export class LwfComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.lwfForm = this.formBuilder.group({
      ptNumber: ['', [Validators.required, Validators.pattern(/^[a-zA-Z0-9]{5,}$/)]],
      filingCycle: ['', Validators.required],
      state: ['', Validators.required],
      ptSlabs: ['', Validators.required]
    });
  }
  ngOnInit(): void {
  ...
  onSubmit() {
    ...
    const body = {
      id: 0,
      // company_Id: this.lwfForm.get('company')?.value,
      name: this.lwfForm.get('allowance_name')?.value,
      type: this.lwfForm.get('type')?.value,
      value: this.lwfForm.get('value')?.value,
  ...
  updateAllowance() {
    ...
    this.submitted = true;
    if (!this.lwfForm.valid || !this.cateId) {
      return;
    }
    this.spinLoader = true;
