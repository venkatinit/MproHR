...
export class ProfessionalTaxComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.ptForm = this.formBuilder.group({
  ...
  onSubmit() {
    ...
    const body = {
      id: 0,
      // company_Id: this.ptForm.get('company')?.value,
      name: this.ptForm.get('allowance_name')?.value,
      type: this.ptForm.get('type')?.value,
      value: this.ptForm.get('value')?.value,
  ...
  updateAllowance() {
    ...
    this.submitted = true;
    if (!this.ptForm.valid || !this.cateId) {
      return;
    }
    this.spinLoader = true;
