...
export class EsicComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.esicForm = this.formBuilder.group({
      esicNumber: ['', [Validators.required, Validators.pattern(/^[0-9]{10}$/)]], // 10 digit ESIC no
      deductionCycle: ['monthly', Validators.required],
      eecr: ['', [Validators.required, Validators.min(0), Validators.max(100)]], // Employee % contribution
      ercr: ['', [Validators.required, Validators.min(0), Validators.max(100)]], // Employer % contribution
      employerDeductsEmployer: [false],
      employerDeductsEmployee: [false]
    }, {
      validators: this.atLeastOneDeductOption() // custom validator
    });
  }
  atLeastOneDeductOption() {
    return (form: FormGroup) => {
      const employer = form.get('employerDeductsEmployer')?.value;
      const employee = form.get('employerDeductsEmployee')?.value;
      return employer || employee ? null : { deductSourceRequired: true };
    };
  }
  ngOnInit(): void {
  ...
  onSubmit() {
    ...
    const body = {
      id: 0,
      // company_Id: this.esicForm.get('company')?.value,
      name: this.esicForm.get('allowance_name')?.value,
      type: this.esicForm.get('type')?.value,
      value: this.esicForm.get('value')?.value,
  ...
  updateAllowance() {
    ...
    this.submitted = true;
    if (!this.esicForm.valid || !this.cateId) {
      return;
    }
    this.spinLoader = true;
