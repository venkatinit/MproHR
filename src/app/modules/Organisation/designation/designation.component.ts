...
export class DesignationComponent implements OnInit, OnDestroy {
  ...
  getDesignations() {
    ...
    this.api.get(`api/accounting/banks/all?${queryParams}`).subscribe((res: ApiResponse<any>) => {
      this.designations_list = Array.isArray(res.data) ? res.data : [res.data];
  ...
  saveDesignation() {
    ...
    const body = {
      ...
      designation_name: this.addDesignation.get('designation_name')?.value,
      created_At: new Date(),
      status: true,
    };
    ...
    this.api.get(`api/accounting/bank/${id}`).subscribe(
      (res: any) => {
        if (res && res.data && res.succeeded && res.data.status) {
          this.addDesignation.controls['bank_name'].setValue(res.data.bank_Name);
  ...
  updateDesignation() {
    ...
    const body = {
      "id": this.cateId,
      "company_Id": companyId,
      "bank_Name": this.addDesignation.get("bank_name").value,
      "branch": this.addDesignation.get("branch_name").value,
      "address": this.addDesignation.get("bank_address").value,
      "account_Number": this.addDesignation.get("account_number").value,
      "account_Type": this.addDesignation.get("account_type").value,
      "bM_Name": this.addDesignation.get("bm_name").value,
      "bM_Contact_No": this.addDesignation.get("bm_contact_no").value,
      "branch_Contact_No": " ",
      "opening_Balance": this.addDesignation.get("opening_Balance").value,
      ...
      if (result.isConfirmed) {
        this.api.delete(`api/accounting/bank/delete/${id}`).subscribe({
          next: (res: any) => {
            ...
            window.location.reload();
            this.getDesignations(); // refresh list without reloading the page
          },
          error: (err: any) => {
