...
export class DepartmentComponent implements OnInit, OnDestroy {
  ...
  getDepartments() {
    ...
    this.api.get(`api/accounting/banks/all?${queryParams}`).subscribe((res: ApiResponse<any>) => {
      this.departments_list = Array.isArray(res.data) ? res.data : [res.data];
  ...
  saveDepartment() {
    ...
    const body = {
      ...
      bank_Name: this.addDepartment.get('bank_name')?.value,
      branch: this.addDepartment.get('branch_name')?.value,
      address: this.addDepartment.get('bank_address')?.value,
      account_Number: this.addDepartment.get('account_number')?.value,
      account_Type: this.addDepartment.get('account_type')?.value,
      bM_Name: this.addDepartment.get('bm_name')?.value,
      bM_Contact_No: this.addDepartment.get('bm_contact_no')?.value,
      branch_Contact_No: '',
      opening_Balance: this.addDepartment.get('opening_Balance')?.value,
    ...
    this.api.get(`api/accounting/bank/${id}`).subscribe(
      (res: any) => {
        if (res && res.data && res.succeeded && res.data.status) {
          this.addDepartment.controls['bank_name'].setValue(res.data.bank_Name);
          this.addDepartment.controls['branch_name'].setValue(res.data.branch);
          this.addDepartment.controls['bank_address'].setValue(res.data.address);
          this.addDepartment.controls['account_number'].setValue(res.data.account_Number);
          this.addDepartment.controls['account_type'].setValue(res.data.account_Type);
          this.addDepartment.controls['bm_name'].setValue(res.data.bM_Name);
          this.addDepartment.controls['bm_contact_no'].setValue(res.data.bM_Contact_No);
          this.addDepartment.controls['opening_Balance'].setValue(res.data.opening_Balance);
          this.submitted = false;
          this.errors = [];
        }
        this.spinLoader = false;
      ...
      if (result.isConfirmed) {
        this.api.delete(`api/accounting/bank/delete/${id}`).subscribe({
          next: (res: any) => {
            ...
            window.location.reload();
            this.getDepartments(); // refresh list without reloading the page
          },
          error: (err: any) => {
